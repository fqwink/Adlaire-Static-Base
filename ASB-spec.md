# セルフホスト型静的コンテンツホスティングシステム仕様書

## 1 本書の位置づけ

本書は、セルフホスト型静的コンテンツホスティングシステム「Adlaire-Static-Base（ASB）」の仕様正本である。
...
| ドメイン管理 | DNS ドメイン割り当て・管理 |
| SSL管理 | Let's Encrypt との統合・証明書自動更新 |
| ファイル管理 | ファイルアップロード・削除・圧縮 |
...
| ファイル操作 | Go 標準 `os`, `io` |
| 圧縮 | Go 標準 `archive/tar`, `compress/gzip`, `compress/brotli` |
| 暗号化 | Go 標準 `crypto` |
| SSL 証明書 | Let's Encrypt（ACME クライアント内製） |
| 対応 OS | Linux |
...
ASB 起動時に以下の統計ディレクトリが自動生成される：

- `.asb/config/` - 設定ファイル（projects.json、backups.json、config.json等）
- `.asb/storage/` - プロジェクトファイルストレージ
- `.asb/logs/` - ログファイル（access.log、error.log等）
- `.asb/certs/` - SSL証明書ストレージ
...
E2E テスト
- 対象：ワークフロー全体
- 検証シナリオ：プロジェクト作成 → ファイルアップロード → 削除、ドメイン割り当て → SSL発行、バックアップ → 復旧
- ツール：bash スクリプト + curl
...
以下はモック・スタブで対応：
- Let's Encrypt との実際の連携（テスト環境では fake ACME サーバーを使用）
