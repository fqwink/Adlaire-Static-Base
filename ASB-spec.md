...
Rev.42 時点では、無料独自SSLのみを XServer Static 互換目標から確定仕様へ昇格する。
...
| 暗号化 | Go 標準 `crypto` |
| SSL 証明書 | SSL証明書管理境界（ACME 実通信・CA選定・証明書自動更新は Rev.42 時点では実装しない） |
| 対応 OS | Linux |
...
- 圧縮：Gzip による自動圧縮
- Brotli は Rev.42 時点では ASB 本体に実装しない
...
Rev.42 時点では、Webhook失敗時の自動リトライスケジュールを実装しない。
...
- バックアップ保存先を別障害領域へ複製する作業は Rev.42 時点ではASB外の運用責務とする
- 外部ストレージ連携は Rev.42 時点では実装対象外とする
...
- 標準出力（stdout）への通常ログ出力は Rev.42 時点では実装しない
...
Rev.42 時点の確定対象は、ASB 本体が提供する HTTP JSON API、および将来 SDK が使用する通信規格である。

SDK 本体、SDK 配布方針、SDK 認証仕様は Rev.42 時点では実装対象外とする。

SDK 通信規格は、ASB 本体が提供する HTTP JSON API と同一とする。

**API 設計原則**
- HTTP + JSON を使用する
- HTTP/2 対応は ASB互換目標として扱い、Rev.42 時点では実装詳細を確定しない
- デフォルト接続境界は `localhost:3000` とする
- 管理 API は Rev.42 時点では認証なしとする
...
- ASB 本体は Rev.42 時点ではインターネット公開用 listen 設定を既定値として提供しない
- リモートアクセスは ASB 外部のリバースプロキシ、VPN、SSH tunnel、ファイアウォール等の運用境界で扱う
- ASB 本体は Rev.42 時点ではリバースプロキシ設定ファイルを生成しない
- SSL/TLS：本番環境では必須とするが、Rev.42 時点では ASB 本体では TLS 終端を実装しない
- TLS 終端は ASB 外部のリバースプロキシまたはロードバランサで扱う

**レート制限**
- Rev.42 時点では ASB 本体に実装しない
...
- エラーログ：エラー・例外・警告
- ログファイル暗号化：Rev.42 時点では ASB 本体に実装しない
...
`latest` 指定、自動最新版選択、未指定バージョンでの実行は Rev.42 時点では禁止する。
...
本章は将来計画の記録であり、Rev.42 時点の実装対象を増やすものではない。

以下は Rev.42 時点では実装対象外とする。

| 対象 | Rev.42 時点の扱い | 実装禁止範囲 |
|-----|------------------|------------|
...
Rev.42 時点では、検討・調査中事項を実装へ反映してはならない。
...
Rev.42 時点の実装対象は、ASB のセルフホスト型静的コンテンツ配信ホスティングに必要なバックエンド機能に限定する。
...
- Gzip 圧縮を実装対象とする
- Brotli 圧縮は Rev.42 時点では ASB 本体に実装しない
...
Rev.42 時点では、SSL 管理は XServer Static 互換目標として無料独自SSLを提供する。

無料独自SSLは、独自ドメイン単位で有効化し、証明書取得、証明書更新、状態確認を ASB 本体が自動実行する。

ACME は利用者向け機能名ではなく、無料独自SSLを実現する内部実装方式である。

Rev.42 時点の ACME は Let’s Encrypt ACME v2 のみに対応する。
...
- Webhook失敗時の自動リトライは Rev.42 時点では実装しない
...
本節は Rev.42 時点の実装契約である。実装者は本節に反する判断をコード側で独自に行ってはならない。
...
`204 No Content` は Rev.42 時点では使用しない。
...
Range request は Rev.42 時点では実装しない。
...
Rev.42 時点では、Webhookデプロイ時の対象ファイルパスはリポジトリルート配下の全静的ファイルとする。
...
本節は Rev.42 時点で API、JSON保存、ログ、起動時検証の入出力を固定する仕様である。
...
`log.format` は Rev.42 時点では `json` のみ許可する。
...
ログ API は `access.log` または `error.log` の現行ファイルのみを読む。ローテーション済みログは Rev.42 時点ではログ API の対象外とする。
...
本節は Rev.42 時点で package 境界、公開 interface、Repository、Storage、複数ファイル更新の実装契約を固定する仕様である。
...
Rev.42 時点では、複数JSON更新に外部トランザクション機構を導入してはならない。
...
Rev.42 の初期実装では、Go 実装ファイルを以下の構成で作成する。
...
Rev.42 の実装では、API と起動時検証が返すエラーコードを以下に固定する。
...
Rev.42 の実装では、実装 package と同じ責務単位でテストファイルを配置する。
...
Rev.42 時点では、ASB 本体の管理 API に認証機能を実装しない。
...
ASB 本体は Rev.42 時点では以下を実装してはならない。
...
Rev.42 時点では、`Authorization` ヘッダーまたは `X-API-Key` ヘッダーの有無により、成功・失敗・レスポンス内容を変えてはならない。
...
SDK 認証仕様は Rev.42 の対象外とし、実装対象へ昇格する場合は事前に `ASB-spec.md` を改訂する。

---

### 13.19 Rate limiting 固定仕様

Rev.42 時点では、ASB 本体に Rate limiting を実装しない。
...
Rev.42 時点では、ASB 本体に Brotli 圧縮を実装しない。

ASB の標準圧縮機能は、Go 標準ライブラリ `compress/gzip` で実装できる Gzip に限定する。

Brotli 圧縮は Go 標準ライブラリに含まれないため、Rev.42 時点では外部ライブラリ例外採用を行わない。
...
Rev.42 時点では、SDK 本体を実装しない。

Rev.42 時点では、SDK 配布方針を確定しない。

Rev.42 時点では、SDK 認証仕様を確定しない。
...
SDK 通信のために、ASB 本体は Rev.42 時点では以下を実装してはならない。
...
ASB 本体は Rev.42 時点では以下の通信方式を SDK 通信として実装してはならない。
...
SDK から ASB 管理 API を呼び出す場合でも、Rev.42 時点では `Authorization` ヘッダー、`X-API-Key` ヘッダー、cookie、セッションIDを認証判断に使用してはならない。
...
Rev.42 時点では、ASB 本体に無料独自SSLを実装する。
...
Rev.42 時点で ASB 本体は以下を実装してはならない。
...
Rev.42 時点では、ASB互換目標は将来の到達目標であり、個別の確定仕様へ昇格した項目のみ実装対象とする。
...
Rev.42 時点で ASB互換目標に含める対象は以下とする。
...
Rev.42 時点で ASB互換目標に含めない対象は以下とする。
...
ASB互換目標に含まれる機能であっても、以下は Rev.42 時点では実装対象ではない。
...
Rev.42 時点では、将来計画、保留事項、検討・調査中事項は実装対象ではない。
...
将来計画機能を理由に、ASB 本体は Rev.42 時点では以下を追加、変更、生成してはならない。
...
静的コンテンツ実体、ログファイル、証明書ファイル、ビルド済みバイナリは、Rev.42 時点のマイグレーション対象外とする。

### 16.3 schemaVersion 固定

各実行時 JSON ファイルはトップレベルに `schemaVersion` を持つ。

Rev.42 時点の `schemaVersion` は `1` とする。
