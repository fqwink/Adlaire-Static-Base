...
ASB互換目標は将来の到達目標であり、Rev.29 時点の実装対象ではない。
...
| 暗号化 | Go 標準 `crypto` |
| SSL 証明書 | SSL証明書管理境界（ACME 実通信・CA選定・証明書自動更新は Rev.29 時点では実装しない） |
| 対応 OS | Linux |
...
- ACME 実通信は Rev.29 時点では ASB 本体に実装しない
...
- 圧縮：Gzip による自動圧縮
- Brotli は Rev.29 時点では ASB 本体に実装しない
...
Rev.29 時点では、Webhook失敗時の自動リトライスケジュールを実装しない。
...
- バックアップ保存先を別障害領域へ複製する作業は Rev.29 時点ではASB外の運用責務とする
- 外部ストレージ連携は Rev.29 時点では実装対象外とする
...
- 標準出力（stdout）への通常ログ出力は Rev.29 時点では実装しない
...
Rev.29 時点の確定対象は、ASB 本体が提供する HTTP JSON API、および将来 SDK が使用する通信規格である。

SDK 本体、SDK 配布方針、SDK 認証仕様は Rev.29 時点では実装対象外とする。
...
`latest` 指定、自動最新版選択、未指定バージョンでの実行は Rev.29 時点では禁止する。
...
- ユーザー認証（Rev.29 時点では実装しない）
- マルチテナント対応
- API キー管理（Rev.29 時点では実装しない）
- SDK 本体・SDK 配布方針・SDK 認証仕様
- Rate limiting（Rev.29 時点では実装しない）
- 外部ストレージサービス統合（AWS S3 等）

### 12.3 検討・調査中事項

- 複数インスタンス時の NFS/分散ストレージ選定
- ACME 実通信、CA選定、ワイルドカード証明書対応、証明書自動更新
...
Rev.29 時点の実装対象は、ASB のセルフホスト型静的コンテンツ配信ホスティングに必要なバックエンド機能に限定する。
...
- Gzip 圧縮を実装対象とする
- Brotli 圧縮は Rev.29 時点では ASB 本体に実装しない
...
Rev.29 時点では、SSL 管理は証明書メタデータ、証明書配置、証明書検証に限定する。

ACME は ASB互換目標に含めるが、ACME 実通信、CA選定、ワイルドカード証明書対応、証明書自動更新は Rev.29 時点では ASB 本体に実装しない。
...
- Webhook失敗時の自動リトライは Rev.29 時点では実装しない
...
本節は Rev.29 時点の実装契約である。実装者は本節に反する判断をコード側で独自に行ってはならない。
...
Range request は Rev.29 時点では実装しない。
...
Rev.29 時点では、Webhookデプロイ時の対象ファイルパスはリポジトリルート配下の全静的ファイルとする。
...
本節は Rev.29 時点で API、JSON保存、ログ、起動時検証の入出力を固定する仕様である。
...
`log.format` は Rev.29 時点では `json` のみ許可する。
...
ログ API は `access.log` または `error.log` の現行ファイルのみを読む。ローテーション済みログは Rev.29 時点ではログ API の対象外とする。

ログには内部ファイルパス、スタックトレース、環境変数、シークレットを含めてはならない。

#### 13.16.8 テスト固定項目

Rev.29 の実装では、以下のテストを必須とする。
...
本節は Rev.29 時点で package 境界、公開 interface、Repository、Storage、複数ファイル更新の実装契約を固定する仕様である。
...
Rev.29 時点では、複数JSON更新に外部トランザクション機構を導入してはならない。
...
Rev.29 時点では、ASB 本体の管理 API に認証機能を実装しない。
...
ASB 本体は Rev.29 時点では以下を実装してはならない。
...
Rev.29 時点では、`Authorization` ヘッダーまたは `X-API-Key` ヘッダーの有無により、成功・失敗・レスポンス内容を変えてはならない。
...
SDK 認証仕様は Rev.29 の対象外とし、SDK通信規格固定後に別途仕様化する。

---

### 13.19 Rate limiting 固定仕様

Rev.29 時点では、ASB 本体に Rate limiting を実装しない。
...
Rev.29 時点では、ASB 本体に Brotli 圧縮を実装しない。

ASB の標準圧縮機能は、Go 標準ライブラリ `compress/gzip` で実装できる Gzip に限定する。

Brotli 圧縮は Go 標準ライブラリに含まれないため、Rev.29 時点では外部ライブラリ例外採用を行わない。
...
Rev.29 時点では、SDK 本体を実装しない。

Rev.29 時点では、SDK 配布方針を確定しない。

Rev.29 時点では、SDK 認証仕様を確定しない。
...
SDK 通信のために、ASB 本体は Rev.29 時点では以下を実装してはならない。
...
ASB 本体は Rev.29 時点では以下の通信方式を SDK 通信として実装してはならない。
...
SDK から ASB 管理 API を呼び出す場合でも、Rev.29 時点では `Authorization` ヘッダー、`X-API-Key` ヘッダー、cookie、セッションIDを認証判断に使用してはならない。
...
ASB 本体は Rev.29 時点では証明書ファイルを生成、取得、更新、削除、失効してはならない。
...
Rev.29 時点で ASB互換目標に含めない対象は以下とする。
...
ASB互換目標に含まれる機能であっても、以下は Rev.29 時点では実装対象ではない。
...
以下はモック・スタブで対応：
- ACME 実通信および CA 連携（Rev.29 時点では実通信を実装対象外とし、SSL管理境界のみ検証）
...
静的コンテンツ実体、ログファイル、証明書ファイル、ビルド済みバイナリは、Rev.29 時点のマイグレーション対象外とする。

### 16.3 schemaVersion 固定

各実行時 JSON ファイルはトップレベルに `schemaVersion` を持つ。

Rev.29 時点の `schemaVersion` は `1` とする。
