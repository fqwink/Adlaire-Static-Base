...
- SDK、CA など ASB互換目標に含めない機能は、仕様確定後に実装対象へ昇格する
...
Rev.1.11 時点の実装対象は、ASB のセルフホスト型静的コンテンツ配信ホスティングに必要なバックエンド機能に限定する。
...
Rev.1.11 時点では、SSL 管理は管理境界とデータモデルを実装対象とし、ACME は ASB互換目標に含める。Let's Encrypt ACME クライアントの実通信は詳細仕様確定後に実装する。
...
以下はモック・スタブで対応：
- Let's Encrypt との実際の連携（Rev.1.11 時点では実通信を実装対象外とし、SSL管理境界のみ検証）
...
| バージョン | 日付 | 内容 |
|-----------|------|------|
| Rev.1.11 | 2026-09-08 | XServer Static 相当仕様を ASB互換目標へ名称変更し、SSL/ACME を ASB互換目標に含める方針へ整理 |
