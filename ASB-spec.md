...
Rev.23 時点では、Webhook失敗時の自動リトライスケジュールを実装しない。
...
- バックアップ保存先を別障害領域へ複製する作業は Rev.23 時点ではASB外の運用責務とする
- 外部ストレージ連携は Rev.23 時点では実装対象外とする
...
- 標準出力（stdout）への通常ログ出力は Rev.23 時点では実装しない
...
Rev.23 時点の確定対象は、ASB 本体が提供する HTTP JSON API である。
...
`latest` 指定、自動最新版選択、未指定バージョンでの実行は Rev.23 時点では禁止する。
...
Rev.23 時点の実装対象は、ASB のセルフホスト型静的コンテンツ配信ホスティングに必要なバックエンド機能に限定する。
...
Rev.23 時点では、SSL 管理は管理境界とデータモデルを実装対象とし、ACME は ASB互換目標に含める。ACME 実通信、CA選定、ワイルドカード証明書対応は詳細仕様確定後に実装対象へ昇格する。
...
- Webhook失敗時の自動リトライは Rev.23 時点では実装しない
...
本節は Rev.23 時点の実装契約である。実装者は本節に反する判断をコード側で独自に行ってはならない。
...
Range request は Rev.23 時点では実装しない。
...
Rev.23 時点では、Webhookデプロイ時の対象ファイルパスはリポジトリルート配下の全静的ファイルとする。
...
本節は Rev.23 時点で API、JSON保存、ログ、起動時検証の入出力を固定する仕様である。
...
`log.format` は Rev.23 時点では `json` のみ許可する。
...
ログ API は `access.log` または `error.log` の現行ファイルのみを読む。ローテーション済みログは Rev.23 時点ではログ API の対象外とする。

ログには内部ファイルパス、スタックトレース、環境変数、シークレットを含めてはならない。

#### 13.16.8 テスト固定項目

Rev.23 の実装では、以下のテストを必須とする。
...
本節は Rev.23 時点で package 境界、公開 interface、Repository、Storage、複数ファイル更新の実装契約を固定する仕様である。
...
Rev.23 時点では、複数JSON更新に外部トランザクション機構を導入してはならない。
...
以下はモック・スタブで対応：
- ACME 実通信および CA 連携（Rev.23 時点では実通信を実装対象外とし、SSL管理境界のみ検証）
...
静的コンテンツ実体、ログファイル、証明書ファイル、ビルド済みバイナリは、Rev.23 時点のマイグレーション対象外とする。

### 16.3 schemaVersion 固定

各実行時 JSON ファイルはトップレベルに `schemaVersion` を持つ。

Rev.23 時点の `schemaVersion` は `1` とする。
