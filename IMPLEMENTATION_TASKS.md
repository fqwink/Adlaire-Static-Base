...
- `:id` は Rev.44 の UUID 正規表現に一致する値のみ許可する。
...
- Rev.44 時点では複数JSON更新に外部トランザクション機構を導入しない。
- Rev.44 時点の API エンドポイント固定表に記載されたメソッド、パス、成功ステータス、失敗コードを実装する。
- Rev.44 API 成功レスポンス固定表に記載された JSON キーと型を実装する。
...
- Range request は Rev.44 時点では実装せず、`Range` ヘッダーを無視して `206 Partial Content` を返さない。
...
- 外部ストレージ連携を Rev.44 時点では実装対象外として扱う。
...
目的: Rev.44 時点で実装対象外の機能が混入していないことを確認する。

### 実装タスク

- 未確定のASB互換目標を将来の到達目標として扱い、Rev.44 時点の実装対象として扱わない。
...
- 将来計画、保留事項、検討・調査中事項を Rev.44 時点の実装対象として扱わない。
