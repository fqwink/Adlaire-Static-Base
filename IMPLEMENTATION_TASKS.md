...
- Rev.83 時点では複数JSON更新に外部トランザクション機構を導入しない。
- Rev.83 時点の API エンドポイント固定表に記載されたメソッド、パス、成功ステータス、失敗コードを実装する。
- Rev.83 API 成功レスポンス固定表に記載された JSON キーと型を実装する。
- Rev.83 API 個別実装契約に記載された request schema、保存先、更新順序、audit 対象を実装する。
- Rev.83 API 別失敗条件固定表に記載された失敗条件、HTTP status、error code を実装する。
...
- Range request は Rev.83 時点では実装せず、`Range` ヘッダーを無視して `206 Partial Content` を返さない。
...
- 外部ストレージ連携を Rev.83 時点では実装対象外として扱う。
...
- ASB SDK の Go 実装を Rev.83 時点では外部配布サービスへ登録しない。
...
目的: Rev.83 時点で実装対象外の機能が混入していないことを確認する。

### 実装タスク

- 未昇格のASB互換目標を将来の到達目標として扱い、Rev.83 時点の実装対象として扱わない。
...
- HTTP/2 が暗黙的に有効化されないよう、Rev.83 の実装では `http.Server.TLSNextProto` を空 map に設定する。
- HTTP/2 専用設定項目、h2c、ALPN独自制御、server push、stream priority、専用handler、専用middleware、専用ログ項目を実装しない。
- 将来計画、保留事項、検討・調査中事項を Rev.83 時点の実装対象として扱わない。
...
- Content Pipeline、Site Routing、Site Rendering、Site Output、Blog、Docs、Sitemap、Ad Slot、Asset Pipeline、Source Sync、External Data Integration、Runtime Cache、Database Gateway、Database Adapter が Rev.83 時点の実装対象へ昇格していないことを確認する。
