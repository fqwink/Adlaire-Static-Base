...
- `:id` は Rev.66 の UUID 正規表現に一致する値のみ許可する。
...
- Rev.66 時点では複数JSON更新に外部トランザクション機構を導入しない。
- Rev.66 時点の API エンドポイント固定表に記載されたメソッド、パス、成功ステータス、失敗コードを実装する。
- Rev.66 API 成功レスポンス固定表に記載された JSON キーと型を実装する。
...
- Range request は Rev.66 時点では実装せず、`Range` ヘッダーを無視して `206 Partial Content` を返さない。
...
- 外部ストレージ連携を Rev.66 時点では実装対象外として扱う。
...
- ASB SDK の Go 実装を Rev.66 時点では外部配布サービスへ登録しない。
...
目的: Rev.66 時点で実装対象外の機能が混入していないことを確認する。

### 実装タスク

- 未昇格のASB互換目標を将来の到達目標として扱い、Rev.66 時点の実装対象として扱わない。
...
- HTTP/2 が暗黙的に有効化されないよう、Rev.66 の実装では `http.Server.TLSNextProto` を空 map に設定する。
- HTTP/2 専用設定項目、h2c、ALPN独自制御、server push、stream priority、専用handler、専用middleware、専用ログ項目を実装しない。
- 将来計画、保留事項、検討・調査中事項を Rev.66 時点の実装対象として扱わない。
...
- Markdown / MDX rendering、JSON Front Matter、Content Collections、File-based Routing、SSG、SSR、Hybrid Rendering、Islands Architecture、Route Manifest、Build Manifest、Blog generator、Documentation generator、Sitemap generator、SEO metadata generator、i18n route generator、Ad Slot Rendering、External Content Loader、Runtime Cache、Database Gateway、Database Adapter が Rev.66 時点の実装対象へ昇格していないことを確認する。
...
- Auteur 由来の Content Pipeline、Site Routing、Site Rendering、Site Component Runtime、Site Runtime、Site Output、Asset Pipeline、Source Sync、External Data Integration、Runtime Cache を実装対象へ昇格する場合は、ASB における正式機能名、API、設定項目、保存JSON、生成ファイル、生成ディレクトリ、外部依存、実行時データ配置、開発リポジトリ非生成、migration、downgrade、テスト条件を仕様改訂で確定する。
- Auteur 由来の Blog output、Documentation / Knowledge Base output、Sitemap、SEO metadata、i18n route、Ad Slot Rendering を実装対象へ昇格する場合は、Site Output として ASB 名称へ再設計し、Auteur 専用名、Auteur 互換モード、Auteur CLI、Auteur runtime を作らないことを仕様改訂で確定する。
