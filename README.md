...
Rev.77 では、実行時データの初期化・自動生成・不足補完を禁止し、起動設定ファイルと `storage.basePath` 配下の実行時 JSON を分離しています。ログは既存ファイルへの追記と仕様で許可されたローテーションのみを行い、監視 API は都度計算・非永続化・非生成に固定しています。
