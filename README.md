...
Rev.87 では、実行時データの初期化・自動生成・不足補完を禁止し、起動設定ファイルと `storage.basePath` 配下の実行時 JSON を分離しています。実装開始条件、仕様不足時の差し戻し、P0着手前の作成可否境界に加え、P0初回実装を最小Go骨格と `asb version` に限定しています。
