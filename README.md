...
Rev.76 では、実行時データの初期化・自動生成・不足補完を禁止し、起動設定ファイルと `storage.basePath` 配下の実行時 JSON を分離しています。ログファイルは起動前の事前配置を必須とし、ASB は既存ログへの追記と仕様で許可されたローテーションのみを行います。
