...
Rev.78 では、実行時データの初期化・自動生成・不足補完を禁止し、起動設定ファイルと `storage.basePath` 配下の実行時 JSON を分離しています。管理 HTTPS JSON API の共通レスポンスヘッダー、request body、routing 失敗、timestamp、静的配信との境界を固定しています。
