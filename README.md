...
Rev.80 では、実行時データの初期化・自動生成・不足補完を禁止し、起動設定ファイルと `storage.basePath` 配下の実行時 JSON を分離しています。Host header は Domain 解決専用、client IP は `Request.RemoteAddr` 由来に固定し、proxy header は信頼境界として使用しません。HTTP server timeout、request size、panic recovery、client disconnect、graceful shutdown、CORS非実装、Health Check をサーバー実行境界として固定しています。
